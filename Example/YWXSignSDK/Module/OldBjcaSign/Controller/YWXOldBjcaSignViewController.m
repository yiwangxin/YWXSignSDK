...
#import <BjcaSignSDK/YWXBjcaSignManager.h>
#import <BjcaSignSDK/YWXBjcaManager.h>
#import <YWXSignFoundation/YWXNetworkManager.h>
#import <YWXSignFoundation/YWXSignFoundation.h>

static NSString *KXBYEnvironmentKeyName = @"xbyEnvironmentKey";
...
- (void)updateInfo {
    ...
    BOOL isExistCert = self.signManager.isCertificateExist;
...
-(void)startCertDownLoad {
    __weak typeof(self) weakSelf = self;
    [YWXBjcaManager.sharedManager downloadCertificateWithPhoneNumber:self.clientInfoView.phoneNumber completion:^(NSString *code, NSString *message, NSDictionary *info) {
...
- (void)startUpdataCert {
    __weak typeof(self) weakSelf = self;
    [YWXBjcaManager.sharedManager updateCertificateWithCompletion:^(YWXSignStatusCode  _Nonnull code, NSString * _Nonnull message, id  _Nullable info) {
...
- (void)startResetCert {
    __weak typeof(self) weakSelf = self;
    [YWXBjcaManager.sharedManager resetCertificatePasswordWithCompletion:^(YWXSignStatusCode  _Nonnull code, NSString * _Nonnull message, id  _Nullable info) {
...
-(void)cleanCert {
    [self.signManager removeLocalCertificate];
...
-(void)setCertStampImage {
    [self.signManager setupSignatureWithFirmIDs:nil navigationBarTintColor:nil navigationBarBackgroundColor:nil completion:^(YWXSignStatusCode  _Nonnull code, NSString * _Nonnull message, id  _Nullable info) {
...
-(void)showSignDataController {
    ...
    signDataController.signButtonClickCallBack = ^(NSArray * _Nonnull uniqueIDs) {
        [self.signManager signWithUniqueIDs:uniqueIDs
...
-(void)showQRCodeScanViewController {
    ...
    [YWXQRCodeScanViewController showQRCodeScanWith:self scanCompletion:^(NSString * _Nonnull result) {
        ...
        /// sdk 二维码页面
        [self.signManager signWithQRString:result completion:^(NSString *code, NSString *message, NSDictionary *info) {
...
-(void)openAutoSign:(NSString *)sysTag {
    __weak typeof(self) weakSelf = self;
    [self.signManager enableAutomaticSignatureWithSysTag:sysTag
...
-(void)getAutoSignInfo {
    __weak typeof(self) weakSelf = self;
    [self.signManager requestAutomaticSignatureInfoWithCompletion:^(NSString *code, NSString *message, NSDictionary *info) {
...
-(void)quitAutoSign:(NSString *)sysTag {
    __weak typeof(self) weakSelf = self;
    [self.signManager disableAutomaticSignatureWithSysTag:sysTag completion:^(NSString *code, NSString *message, id info) {
...
-(void)openFreePin:(NSString *)days {
    ...
    [self.signManager enablePasswordLessSignatureWithDays:day completion:^(YWXSignStatusCode  _Nonnull code, NSString * _Nonnull message, NSDictionary * _Nullable info) {
...
/// 关闭免密
-(void)closeFreePin {
    [self.signManager disablePasswordLessSignature];
...
-(void)openBiometricsStatus {
    [self.signManager enableBiometricAuthenticationForSignatureWithCompletion:^(NSString *code, NSString *message, id info) {
...
-(void)closeBiometricsStatus {
    [self.signManager disableBiometricAuthenticationForSignatureWithCompletion:^(NSString *code, NSString *message, id info) {
...
-(void)showUserInfo {
    ...
    [self.signManager requestUserInfoWithCompletion:^(NSString *code, NSString *message, id info) {
        if ([code isEqualToString:@"0"]) {
            ...
            userInfoController.freePin = self.signManager.isPasswordLessSignatureEnabled;
...
-(void)showCertDetail {
    __weak typeof(self) weakSelf = self;
    [self.signManager showCertificateDetailWithNavigationBarTintColor:nil navigationBarBackgroundColor:nil completion:^(YWXSignStatusCode  _Nonnull code, NSString * _Nonnull message, id  _Nullable info) {
...
- (void)checkIsExistCert {
    BOOL isCertificateExist = [self.signManager isCertificateExist];
